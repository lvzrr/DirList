...
## Usage
Just add the .exe folder to your path and type `ls`, feel free to change the source code and then rebuilding with: 
```bash
go build ls.go
```
...
### Performance 
In terms of performance, at least in my machine, matches the windows `dir` command in windows 11. Sometimes even surpasses it in speed, with an average runtime of around 0.05 seconds 
